...
function middleware(options = {}) {
  ...
  const createClientBundle = () => {
    ...
    if (productionMode) {
      ...
      const file = files.find(name => name.endsWith(`-${options.fileIndexJs}`));
      clientBundle = require(path.join(options.dirBuildClient, file));
    } else {
      const hash = compiler.records.hash;
      const file = `${hash}-${options.fileIndexJs}`;
      const content = compiler.outputFileSystem.readFileSync(path.join(compiler.outputPath, file));
