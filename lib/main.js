...
function middleware(options = {}) {
  ...
  if (productionMode) {
    ...
    app.get('/', (req, res) => {
      const html = getCompiler(options.fileIndexHtml)
        .outputFileSystem.readFileSync(path.join(compiler.outputPath, options.fileIndexHtml), 'utf8');
  ...
  const getClientModule = (file) => {
    ...
    if (productionMode) {
      ...
      const content = getCompiler(file)
        .outputFileSystem.readFileSync(path.join(theCompiler.outputPath, file), 'utf8');
  ...
  app.use((req, res, next) => {
    ...
    if (context.url) {
      ...
      let html = getCompiler(options.fileIndexHtml)
        .outputFileSystem.readFileSync(path.join(compiler.outputPath, options.fileIndexHtml), 'utf8');
